...
% MS-HLMOv2 for multi-source/multi-modal images matching/registration.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
close all; clc; clear;
...
key_type = 'PC-ShiTomasi';  % Harris, ShiTomasi, PC-Harris, default: PC-ShiTomasi
...
thresh    = 0;    % Harris response threshold, default: 0
radius    = 2;    % Local non-maximum suppression radius, default: 2
Npoint    = 5000; % Keypoints number threhold, default: 5000
...
tic,keypoints_1 = Detect_Keypoints(I1,6,thresh,r1,Npoint,nOctaves1,G_resize,key_type,1);
    t(1)=toc; fprintf(['已完成参考图像特征点检测，用时 ',num2str(t(1)),'s\n']);
              fprintf([' Done keypoints detection of reference image, time: ',num2str(t(1)),'s\n']);
tic,keypoints_2 = Detect_Keypoints(I2,6,thresh,r2,Npoint,nOctaves2,G_resize,key_type,1);
...
tic,descriptors_1 = Multiscale_Descriptor(I1,keypoints_1,patchsize,NBS,NBO,...
...
tic,descriptors_2 = Multiscale_Descriptor(I2,keypoints_2,patchsize,NBS,NBO,...
